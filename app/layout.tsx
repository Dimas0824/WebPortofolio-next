      ...
      <head>
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: `(function(){var A=["bis_skin_checked","bis_register","data-google-query-id","data-new-gr-c-s-check-loaded","data-gr-ext-installed","data-lt-installed","data-lt-tmp-id","bis_disable_all","bis_skin_checked_v2"];var P=["__processed_"];function strip(el){if(!el||el.nodeType!==1)return;var a=el.attributes;for(var i=a.length-1;i>=0;i--){var n=a[i].name,h=false;if(A.indexOf(n)>=0){h=true;}else{for(var m=0;m<P.length;m++){if(n.indexOf(P[m])===0){h=true;break;}}}if(h)el.removeAttribute(n);}}var o=new MutationObserver(function(ms){for(var i=0;i<ms.length;i++)strip(ms[i].target);});o.observe(document.documentElement,{subtree:true,attributes:true});function sw(){strip(document.documentElement);strip(document.body);var e=document.getElementsByTagName("*");for(var i=0;i<e.length;i++)strip(e[i]);}if(document.readyState!=="loading"){sw();}else{document.addEventListener("DOMContentLoaded",sw);}setTimeout(function(){o.disconnect();},5000);})();`,
          }}
        />
        {/* GA4 */}
