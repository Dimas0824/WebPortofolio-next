  ...
  return (
    <section id="stack" className="py-24">
      <div className="mx-auto max-w-[760px] px-4 md:px-6">
        ...
        <h2 className="mt-3 mb-8 font-v2-display text-5xl font-bold uppercase md:text-6xl">
          Tool yang digunakan
        </h2>
        {groups.map((g, i) => (
          <input
        ...
        <div className="v2-stackrow mb-6 flex flex-wrap justify-center gap-2 border-b border-v2-hairline pb-5">
          {groups.map((g, i) => {
            ...
            return (
              <label
                key={g.category}
                htmlFor={`stk-${i}`}
                className="inline-flex cursor-pointer select-none items-center gap-2 rounded-full border border-v2-hairline px-4 py-2 font-v2-mono text-[10px] uppercase tracking-[.18em] text-v2-muted transition-colors"
              >
          ...
          >
            {g.items.map((x) => (
              <li key={x} className="flex items-center gap-4 py-3">
                <span className="font-v2-mono text-[13px] uppercase tracking-[.08em] text-v2-ink">
                  {x}
                </span>
                <span className="ml-auto font-v2-mono text-[9px] uppercase tracking-[.2em] text-v2-muted">
                  {(META[g.category] ?? { tag: g.category }).tag}
                </span>
              </li>
            ))}
          </ul>
